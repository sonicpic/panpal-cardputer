#!/usr/bin/env python3
"""Pack a Codex v2 pet atlas into tiny flash-only Cardputer animation data.

The desktop pet remains the source of truth (1536x2288, 8x11 cells).  This
adapter selects the five rows useful on a 240x135 display, scales each frame to
72x72, quantizes the whole set to one 15-colour palette, and emits row-safe RLE
for direct drawing from ESP32 flash.  ``--demo`` creates a deterministic
development mascot so firmware work never depends on image-generation access.
"""
...
SPECS = (
    AnimationSpec("idle", "Idle", 0, (260, 120, 120, 160, 160, 340)),
    AnimationSpec("running", "Running", 7, (120, 120, 120, 120, 120, 220)),
    AnimationSpec("waiting", "Waiting", 6, (160, 160, 160, 160, 160, 260)),
    AnimationSpec("ready", "Ready", 3, (150, 150, 150, 300)),
    AnimationSpec("blocked", "Blocked", 5, (140, 140, 140, 140, 140, 140, 140, 260)),
)
...
def load_atlas(args: argparse.Namespace) -> Image.Image | None:
    ...
    if args.pet_dir:
        ...
        if manifest.get("spriteVersionNumber") != 2:
            raise SystemExit(f"{manifest_path}: spriteVersionNumber must be 2")
    ...
    if image.size != ATLAS_SIZE:
        raise SystemExit(f"{atlas_path}: expected {ATLAS_SIZE}, got {image.size}")
    return image
