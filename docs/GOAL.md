...
3. **Codex 助手**:在设备屏幕上查看 Mac 上 Codex CLI 的 session 状态与宠物动画,任务完成或需要处理时设备提醒。当前产品不接入 Claude,也不在设备上保留无功能的占位入口。
...
屏幕主页:
  左侧 Codex 动态卡片 —— 当前会话宠物状态;点击进入会话跟随/切换、限额与任务详情
  右侧设置入口       —— WiFi / 电脑(配对) / 亮度 / 熄屏时间;点击进入各自详情页
```
...
- 会话目录、账户模式与周/5h限额来自官方 Codex App Server；CardBridge 每 2 秒轮询一次 `thread/list`，因此即使没有启用或信任 Hook，也能发现桌面端最近会话并依据 `recencyAt` 跟随新 Prompt。CardBridge 与 Codex Desktop 使用独立 App Server，跨进程的即时状态和具体活动文案仍由官方 Hooks 补强；账户/限额使用独立 30 秒刷新节奏。
- 公开 `agentMessage`、文件变更和工具事件提供具体实时文案。桥接明确忽略 reasoning 通知，只保留 ID、裁剪后的标题、项目目录末级、状态和安全短活动说明；不读取完整 transcript、原始命令/工具参数、命令输出或 `auth.json`。
- 最近 8 个会话按最后一次用户消息时间倒序排列。宠物默认跟随最新收到用户消息的会话；后台工具或输出不得重排历史、不得抢屏。设备左右键可临时浏览其他会话，只有后续新的用户消息才恢复自动跟随。
...
- Codex 详情页不画全局状态栏，240×135 屏幕使用 4px 外边距和 4px 中缝，左右各 114px。键盘透传仅以左上角 18×12 微图标保留；当前历史位置用约 24×12px 的 `1/8` 徽标显示在左半屏顶部居中，不挤占右侧标题宽度。
- 左侧只显示宠物动画：构建期仍打包 72×72、共享 16 色、逐行 RLE 资源，运行时按行段最近邻放大到 100×100，不解码 PNG/WebP，也不创建第二张全屏 Canvas。
- 右侧依次显示：106×18 单行滚动标题、106×70 最多四行实时公开文案、两条各 10px 高的 `WEEKLY` / `5H` 紧凑限额行。标题与正文使用原生 13px 中文 BFF，正文采用 17px 行距；禁止再用分数缩放压缩 15px 字体，以免逐字 advance 取整造成字距拥挤。
- 限额有三种明确语义：`subscription` 显示真实比例；`api` 显示低饱和、缓慢流动的满格彩虹渐变和完整 `∞`（仅代表不适用 ChatGPT 周/5h 窗口）；`unknown` 显示灰色 `--`，不得把读取失败伪装成无限。
- 提交 Prompt/公开输出显示 Waiting/Thinking，工具执行才显示 Running；键盘透传开启后页面和 10 FPS 动画继续刷新，但设备按键全部发往 Mac。

## 4. 里程碑与验收标准

| # | 内容 | 验收(由 Codex 与用户实机执行) |
|---|------|--------------------------|
...
| M5 | Codex 会话宠物 | 无 Hook 时约2秒内发现并跟随最新用户 Prompt;后台输出不抢屏;左右浏览最近8会话且显示 `N/8`;状态动画/任务短文/周限额正确;无5h数据安全降级;长标题滚动;Hook需用户显式信任后才提供更即时、完整的跨进程状态 |
...
- Flash 8MB;当前 app 分区 3.3MB,Codex 宠物版固件 2,271,290 字节(68.0%),剩余 1,071,046 字节。
...
- **开发约束(重要)**:
  - Codex 可以写代码、运行 `pio run`、执行 `pio run -t upload` 烧录、使用 `/dev/cu.usbmodem*` 串口并完成实机联调。USB 复位后端口名可能变化,应优先自动探测而非硬编码端口,并避免多个进程同时占用串口。
...
4. **HWCDC 串口抖动**(端口偶发消失几秒)→ 烧录与串口监视流程需等待端口重新出现,并重新自动探测端口。
